...
pub fn verify_chain(cert_path: &str, ca_path: &str) -> Result<VerifyResult> {
    let cert_data =
        std::fs::read(cert_path).with_context(|| format!("can't read {}", cert_path))?;
    let ca_data = std::fs::read(ca_path).with_context(|| format!("can't read {}", ca_path))?;

    let certs =
        parse_pem_certs(&cert_data).with_context(|| format!("bad certificate: {}", cert_path))?;
    let ca_certs =
        parse_pem_certs(&ca_data).with_context(|| format!("bad CA bundle: {}", ca_path))?;
